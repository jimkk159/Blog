        ...
        <ul className="flex h-full w-full space-x-4">
          ...
          <div className="flex w-full flex-col items-start justify-start">
            <div className="flex items-center justify-center space-x-2">
              <div
                className="flex w-full text-self-pink-500 hover:text-self-pink-600"
            ...
            <div
              className="my-1 line-clamp-3 w-full flex-grow overflow-ellipsis pr-1 text-justify text-xs text-self-gray hover:text-zinc-500 cursor-pointer"
              onClick={() => navigate(`/posts/${post.id}`)}
            >
              {post.summary}
            </div>
    ...
    <div className="flex w-full items-start justify-start space-x-4 border-b border-self-gray py-2">
      ...
      <div className="flex h-28 w-full flex-col items-start space-y-2 ">
        <div className="flex h-full w-full flex-col items-start justify-start ">
          <div className="flex items-center justify-center space-x-2">
            <div
              className="flex w-full cursor-pointer text-self-pink-500 hover:text-self-pink-600"
              onClick={() => navigate(`/profile/${post.Author.id}`)}
            >
              <AiFillCheckCircle className="h-[14px] w-[14px]" />
              <p className="ml-0.5 truncate overflow-ellipsis text-xs font-bold">
                {post.Author.name}
              </p>
            </div>
            <p className="overflow-ellipsis whitespace-nowrap text-xs italic">
          ...
          <div
            className="line-clamp-2 w-full cursor-pointer overflow-ellipsis pr-16 text-xs text-self-gray hover:text-zinc-500"
            onClick={() => navigate(`/posts/${post.id}`)}
          >
            {post.summary}
          </div>
